...
export default function ConversationChatClient({ conversationId, initialMessages, initialAiCalls, canSend = false }) {
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        const res = await fetch(`/api/admin/conversations/${conversationId}/messages?after_id=${encodeURIComponent(String(lastId))}`, {
          cache: "no-store",
        });
        if (!res.ok) return;
      ...
      try {
        const res2 = await fetch(`/api/admin/conversations/${conversationId}/ai-calls?after_id=${encodeURIComponent(String(lastAiId))}`, {
          cache: "no-store",
        });
        if (!res2.ok) return;
